...
export function GrblProvider({ children }) {
  ...
  const sendRealtime = useCallback(async (text) => {
    if (!writerRef.current || !connectedRef.current) return false
    try {
  ...
  const processQueue = useCallback(async () => {
    if (!connectedRef.current || awaitingAckRef.current || !writerRef.current || streamRef.current.paused) return
  ...
  }, [baudRate, connected, connecting, disconnect, readLoop, sendRealtime, serialSupported])
  ...
  const sendCommand = useCallback((line) => {
    if (!connectedRef.current) {
      setLastError('Connectez le système avant d’envoyer une commande.')
      return false
    }
    enqueue(line)
    return true
  ...
  const startStream = useCallback((lines, source = 'stream') => {
    if (!connectedRef.current) {
      setLastError('Connectez le système avant de lancer un envoi.')
      return false
    }
